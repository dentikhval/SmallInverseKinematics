...
/*
  ...
  So far I an getting a stable round-trip error of less than 0.001 when feeding IK otput directly to FK input.
  ...
  Option 2 (clean)- detect high calculated error and reject command before executing completely. Can be circumvented by means of the above.
...
#define PULSE_LENGTH_MICROSECONDS 2 // Minimum pulse length to stepper drivers. Warning: if set too high, moveMotors() may not keep up with time constraints coordinatedMove() imposes. If set too low, driver may not register or miss steps.
#define DEFAULT_SPEED 25 // Speed in degrees per second.
#define DEFAULT_ACCELERATION 75 // Acceleration in steps per second per second.
...
/*
 * For the future: steps per degree. Currently unused.
 * 
// Maybe this is better defined by steps per degree, like everyone does - essentially the same as 1/dps[]
// motor_steps * microstep * reduction (gear on axis / gear on motor)
// divided by
// 360.0
const double stepsPerDegree[6] = {
  (200.0 * 1.0 * 1.0) / 360.0,
  (200.0 * 1.0 * 1.0) / 360.0,
  (200.0 * 1.0 * 1.0) / 360.0,
  (200.0 * 1.0 * 1.0) / 360.0,
...
// Define Motor Degrees per step for 6 joints
// actual movement degrees per issued step pulse - only used in driveMotors function
// Only used in moveMotors(), and in a VERY strange way: manipulating steps until difference between target and current angle (!)  is below angle per step
const double dps[6] = {
  360.0 / 200.0 / 1.0 / 1.0, // dps[0]
  360.0 / 200.0 / 1.0 / 1.0,
  360.0 / 200.0 / 1.0 / 1.0,
  360.0 / 200.0 / 1.0 / 1.0,
  360.0 / 200.0 / 1.0 / 1.0,
...
// Last issued STEP signal state
boolean PULstat[6] = {0, 0, 0, 0, 0, 0};
...
void setup()
{
  ...
  for (int i = 0; i < numMotors; i++) {
    ...
    digitalWrite(stepPins[i], LOW); // gear ratio = 96/20 = 4.8
    digitalWrite(dirPins[i], LOW); //LOW = negative direction
...
void loop()
{
  ...
  for (int j = 0; j < numPoints; j++) { // repeat for the defined number of points
    ...
    InverseK(X_CPY, JA); // IT MODOFIES BOTH ARGUMENTS!
  ...
  } // for specified number of data points

  /*
       // At this point, why not just use a ready coordinated motion library?
    //Perform the moves by moving the motors (INCREMENTAL - watch start position closely) (TIME BASED - WILL BREAK WITH INTERRUPTS!!!)

    // Local variable: speed and acceleration. Only used in the 20 lines below. Divided by 1 million because human-readable
    // value is steps per second, and machine expects steps per MICROsecond.
    double velG = DEFAULT_SPEED / 1000000;
    double accG = DEFAULT_ACCELERATION / 1000000;
    // coordinatedMove(Start joint angles, end joint angles, speed, acceleration, start velocity, end velocity)
    // coordinatedMove(double* xfi, double* xff, double vel0, double acc0, double velini, double velfin){

    coordinatedMove(Jhome, J1, velG, accG, 0.0, 0.0);

    coordinatedMove(J1,  J11, velG, accG, 0.0,	  0.5 * velG); // Move through waypoints. Keep half speed when passing waypoints (last 2 arguments), don't perform a full decelleration.
    coordinatedMove(J11, J12, velG, accG, 0.5 * velG, 0.5 * velG);
    coordinatedMove(J12, J13, velG, accG, 0.5 * velG, 0.5 * velG);
    coordinatedMove(J13, J14, velG, accG, 0.5 * velG, 0.5 * velG);
    coordinatedMove(J14, J15, velG, accG, 0.5 * velG, 0.5 * velG);
    coordinatedMove(J15, J16, velG, accG, 0.5 * velG, 0.5 * velG);
    coordinatedMove(J16, J17, velG, accG, 0.5 * velG, 0.5 * velG);
    coordinatedMove(J17, J1,  velG, accG, 0.5 * velG, 0.0);

    coordinatedMove(J1,  J18, velG, accG, 0.0,	  0.8 * velG);
    coordinatedMove(J18, J14, velG, accG, 0.8 * velG, 0.0);
    coordinatedMove(J14, J1,  velG, accG, 0.0, 	  0.0);

    coordinatedMove(J1, J2, velG, accG, 0.0, 0.0);
    coordinatedMove(J2, J1, velG, accG, 0.0, 0.0);

    coordinatedMove(J1, J3, velG, accG, 0.0, 0.0);
    coordinatedMove(J3, J1, velG, accG, 0.0, 0.0);

    coordinatedMove(J1, J4, velG, accG, 0.0, 0.0);
    coordinatedMove(J4, J1, velG, accG, 0.0, 0.0);

    coordinatedMove(J1, Jhome, velG, accG, 0.0, 0.0);
    //--------------------GoGoGoBack---------------
  */
...
/*
  Generally, this is an OK solution to control stepper movement. HOWEVER, it relies on time, and interrupts from stuff like serial comm or others may cause lost steps.
  3D printer firmwares generally do a good job of coordinated movement - maybe put those to good use?
  Ahh well... Can just freeze all serial comms while arm is moving

  Pass acc0 or vel0 = 0? You will hang the processor.
  Maybe if we pass initial or final velocity higher than default velocity this may cause issues.
*/
void coordinatedMove(double * xfi, double * xff, double vel0, double acc0, double velini, double velfin) {

  // Find maximum joint angle difference (longest travel)
  ...
  lmax = max(lmax, abs(xff[5] - xfi[5])); // Finally, max of all

  // Checks to prevent passing higher initial or final velocity than main velocity
  velini = min(vel0, velini);
  velfin = min(vel0, velfin);

  unsigned long preMil = micros();

  // "l" - multiplier of the full motion distance to be used within the time-constrained while loop in this function
  // to calculate the PORTION of the move to do within the mini-loop
  double l = 0.0;

  // limited by either set max speed or calculated speed (whichever is smaller)
  // Calculated speed: max travel (of all axes)* acceleration + 0.5 * initial velocity ^2 + 0.5 * final velocity ^2
  vel0 = min(vel0, sqrt(lmax * acc0 + 0.5 * velini * velini + 0.5 * velfin * velfin));

  // In reality, I admit this is a good and working method, BUT when micros() rolls over within this loop by any chance the precision of this calculation can
  // potentially be
  // Need to add a check if curMil - preMil is >0 to easily catch this rollover
  unsigned long curMil = (unsigned long) micros(); // SAFETY: if on some exotic Arduino platform micros() does not return an unsigned long, force it to be one.
  unsigned long microsHalfMax = 4294967295 / 2; // 4294967295 is unsigned long maximum value
  unsigned long t = 0;
  // Some black magic to implement trapezoidal motion planning with settable initial and final velocity
  double tap = vel0 / acc0 - velini / acc0; // time of apoapsis??
  double lap = velini * tap + acc0 * tap * tap / 2.0;
  double lcsp = lmax - (vel0 * vel0 / 2.0 / acc0 - velfin * velfin / 2.0 / acc0); // length of move with continuous speed?
  double tcsp = (lcsp - lap) / vel0 + tap;
  double tfin = vel0 / acc0 - velfin / acc0 + tcsp; // finish time?

  /*
     Using time for controlling motor movement... Dangerous!
     What if the minimal step microseconds is set too high and /or the moveMotors() function
     does not keep up to the calculated time constraint?!
     On the other hand, it may help to prevent freezes due to incorrect arguments to some extent
     Added a check if curMil - preMil is >0 to easily catch micros() rollover
     result of unsigned subtraction is still unsigned and thus cannot be negative.
     BUT the result of such substraction is off by 1 microsecond. Not good if you are timing your steppers with this input.

     In theory, this arrangement MAY even work fine with interrupts too - just need to keep ISRs super short.

    Essentially, what this does is breaks the move into 3 stages: acceleration, stable speed and decelleration.
    Then calculates the time and number of steps neded to go through each of these stages.
    HOWEVER, the steps are always administered at the maximum possible step rate, leading to all of them being sent in one batch and then just
    waiting for the time for the next stage to come. Rinse and repeat. What if the motors are so fast that they perform the move sooner than the
    time for the next movement phase comes?
    What if the move is very slow and very long?
    The effect is compounded by doing that to 6 motors, one at a time (!), leading to an uneven start of movement in every one (of three)
    phases of motion.

    Verdict: this is NOT coordinated movement at all!

    What can be done is^
    1 - the delay between high/low pulses for each motor must be adjusted individually for all 6 motors to ensure smooth stepping from the beginning
    to the end of each phase of movement

    2 - steps need to be administered to motors in an asynchronous way simultaneously: e.g. synced first step, and then all steps given out with
    various delays, all leading to motion at the right speed for each individual motor.
    Long story short, use Teensy 4.1 and AccelStepper, or copy code from some 3D printer firmware.
  */

  while (curMil - preMil <= tfin || curMil - preMil > microsHalfMax) {

    if (curMil - preMil < microsHalfMax) { // If there is no rollover
      t = curMil - preMil; // calculate t normally
      if (
        //acceleration phase - naturally the first one to happen because t is initialized with zero
      if (t <= tap) {
        l = velini * t + acc0 * t * t / 2.0; // acceleration: increase target (length?) with time
      }
    ...
    if (t > tap && t <= tcsp) {
      ...
      //deceleration phase
      if (t > tcsp) {
      l = lcsp + vel0 * (t - tcsp) - acc0 * (t - tcsp) * (t - tcsp) / 2.0; // Decceleration: decrease target (length?) with time
      }

      //trajectory x and y as a function of l
      // Xx
      // Incremental distance to travel within this while loop in natural units of the machine (degrees / mm)
      double unitsDelta[6];
      for (int i = 0; i < 6; i++) {			 // Efficiency.
      Xx[i] = xfi[i] + (xff[i] - xfi[i]) / lmax * l; // initial position + PORTION of distance between initial position and target
      }

      // Administer the calculated amount of steps to the motor quickly,
      driveMotors(Xx); // Move motors inside while() loop!
      curMil = (unsigned long)micros();
               // If there has been a rollover, premil is a very high value while curmil is a very low value. When you substract a big unsigned long from a small unsigned long with rollover, you get a result that is off by +1.
    } else {
      t = curMil - preMil - 1; // WTF why it just works? Maybe because we are working with 3 unsigned long's
    }
  } // while time calculated for this move has not yet passed
} // coordinatedMove

/*
   Move Motors function: Administer the required amount of steps in the correct direction, without coordination
   and simply motor-by-motor. Skyentific does away with this because he uses the previous function coordinatedMove
   to break down the move into many stages
*/
void driveMotors(double * Jf) {

  // This function looks like an uncoordinated move, andit will do just that if called separately.
  // In theory, the move will be coordinated if called from coordinatedMove function.
  // Only used in moveMotors(), and in a VERY strange way: manipulating steps until difference between target and current angle (!)  is below angle per step


  //execution
  int delF = PULSE_LENGTH_MICROSECONDS;

  for (int i = 0; i < numMotors; i++) {
    // joints 1-6
    if (Jf[i] - currentPositions[i] > 0.0) { // positive direction of rotation
      digitalWrite(dirPins[i], HIGH);
      while (Jf[i] - currentPositions[i] > dps[i] / 2.0) { // but why divide by 2? I think because one more step would cause an overshoot by more than we currently undershoot, and by NOT making a step we arrive closer to the target.
        if (PULstat[i] == 0) {
        ...
        //currentPositions[i] = Jf[i];
        currentPositions[i] = currentPositions[i] + dps[i] / 2.0;
        if (Jf[i] - currentPositions[i] > dps[i] / 2.0) { // if position still has not been reached... (the same as condition in while()function)
          delayMicroseconds(delF);
        }
      }
    } else {
      digitalWrite(dirPins[i], LOW);
      while (currentPositions[i] - Jf[i] > dps[i] / 2.0) {
        if (PULstat[i] == 0) {
          digitalWrite(stepPins[i], HIGH);
          PULstat[i] = 1;
        } else {
        ...
        //currentPositions[i] = Jf[i];
        currentPositions[i] = currentPositions[i] - dps[i] / 2.0;
        if (currentPositions[i] - Jf[i] > dps[i] / 2.0) {
          delayMicroseconds(delF);
        }
      }
...
void ForwardK(double * Jfk, double * Xfk)
{
  ...
  */
  Xfk[3] = Xfk[3] * 180.0 / PI; // turn -89 into +91 and +89 into -91
  Xfk[4] = Xfk[4] * 180.0 / PI; // strangely, this calculates correctly
  Xfk[5] = Xfk[5] * 180.0 / PI; // turn -89 into +91 and +89 into -91
}
